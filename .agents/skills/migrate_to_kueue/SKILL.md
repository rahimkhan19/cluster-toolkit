...
- **Variable Escaping**: Because the pod's bash script is embedded inside a heredoc, you MUST carefully escape ALL container-side bash variables (e.g., `\$$BLUEPRINT`, `\$$DEPLOYMENT_NAME`, `\$$ANSIBLE_PID`, `\$$PROJECT_ID`) with `\$$` so they are evaluated dynamically inside the pod, rather than prematurely by the host shell generating the YAML. This is especially critical for variables passed to `ansible-playbook --extra-vars` (like `project=\$$PROJECT_ID build=$$BUILD_ID_SHORT`); if container-side variables are not escaped, the host shell evaluates them to empty strings, which will later cause `gcluster create` to fail with "deployment variable project_id was not set". Note: Variables defined by the host shell generating the YAML (like `BUILD_ID_SHORT`) should use `$$` so the host shell evaluates them! Verify each line carefully to ensure proper escaping context!
