...
- **Regular `env` Variables**: If the legacy step used standard `env` variables (e.g., `PROJECT_ID`, `BUILD_ID`, `NUM_NODES`), you MUST copy them into the Pod's `containers[0].env` array in the Job manifest. Ensure you use the exact name `BUILD_ID` (and NOT `CLOUD_BUILD_ID`, which may mistakenly appear in older examples) because scripts inside the Pod (like `find_available_zone.sh`) explicitly expect the `BUILD_ID` variable and will fail with an `unbound variable` error if omitted or renamed!
