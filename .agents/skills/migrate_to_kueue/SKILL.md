...
- **CRITICAL: Lock Name Length Limit**: The `dummy-device-plugin` encodes the lock name (e.g., `test-locks/my-test-name`) into base64 to create a Unix domain socket. Because Linux enforces a strict 108-character limit on Unix socket paths, your lock name MUST NOT exceed roughly 30 characters (excluding the `test-locks/` prefix). If your test name is longer (e.g., `ml-a3-megagpu-onspot-slurm-ubuntu`), you MUST truncate it when defining the lock (e.g., `test-locks/ml-a3-megagpu-onspot-slurm`) to prevent the device plugin from failing to start with a `bind: invalid argument` error.
