...
- **`OPTIONS_BUCKET` Logic**: If the legacy pipeline constructs an `OPTIONS_GCS_PATH` (either dynamically or via a hardcoded bucket like `hpc-ctk1357`), you MUST place a bash `if` statement in the `generate-job-manifest` step *before* the heredoc to dynamically determine the `OPTIONS_BUCKET` based on `PROJECT_ID`. Ensure you map `hpc-toolkit-gsc` to `hpc-ctk1357-gsc` and other projects to the original bucket. Then, construct the path dynamically in the Pod's `env` array (e.g., `value: "gs://$${OPTIONS_BUCKET}/a3uoptions.txt"`).
- **Variable Escaping**: Because the pod's bash script is embedded inside a heredoc, you MUST carefully escape ALL container-side bash variables (e.g., `\$$BLUEPRINT`, `\$$DEPLOYMENT_NAME`, `\$$ANSIBLE_PID`, `\$$PROJECT_ID`, `\$$BUILD_ID_SHORT`) with `\$$` so they are evaluated dynamically inside the pod, rather than prematurely by the host shell generating the YAML. This is especially critical for variables passed to `ansible-playbook --extra-vars` (like `project=\$$PROJECT_ID build=\$$BUILD_ID_SHORT`); if they are not escaped, the host shell evaluates them to empty strings, which will later cause `gcluster create` to fail with "deployment variable project_id was not set". Verify each line carefully to ensure no variable misses this escaping!
